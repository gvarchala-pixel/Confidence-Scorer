...
| Message types | [LangChain Core](https://python.langchain.com/docs/get_started/introduction), provides `HumanMessage`, `AIMessage`, `ToolMessage` |
...
- `OLLAMA_BASE_URL` is the local URL for your Ollama server (e.g. `http://127.0.0.1:11434`)
- `LLM_MODEL` is the model name to use (e.g. `llama3.2`)
- `WEB_SEARCH_PROVIDER` is optional, defaults to `duckduckgo`. Options: `duckduckgo`, `tavily`, `brave`, `google`, `bing`
